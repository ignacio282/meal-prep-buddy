...
describe("NewRecipeFormFields", () => {
  it("filters existing tag suggestions and adds a selected tag", () => {
    ...
    const hiddenTags = container.querySelectorAll('input[type="hidden"][name="tags"]');
  ...
  it("filters ingredients and shows an add action for a new ingredient", () => {
    render(
      <NewRecipeFormFields
        cuisineOptions={cuisineOptions}
        existingIngredients={["Chicken breast", "Cooked rice", "Broccoli florets"]}
        existingTags={["High protein"]}
        proteinOptions={proteinOptions}
      />,
    );

    fireEvent.change(screen.getByPlaceholderText("Search or add an ingredient"), {
      target: { value: "rice" },
    });

    expect(screen.getByRole("button", { name: "Cooked rice" })).toBeInTheDocument();
    expect(
  ...
  it("adds ingredients and steps, removes them, and keeps hidden inputs in sync", () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Season and sear the chicken."), {
    ...
    expect(screen.getByText("Season and sear the chicken.")).toBeInTheDocument();

    let ingredientInputs = container.querySelectorAll(
      'input[type="hidden"][name="ingredients"]',
    );
    let stepInputs = container.querySelectorAll('input[type="hidden"][name="steps"]');
    ...
    fireEvent.click(screen.getByRole("button", { name: "Remove ingredient Cooked rice" }));
    fireEvent.click(screen.getByRole("button", { name: "Remove step 1" }));

    ingredientInputs = container.querySelectorAll('input[type="hidden"][name="ingredients"]');
    stepInputs = container.querySelectorAll('input[type="hidden"][name="steps"]');

    expect(screen.queryByText("Season and sear the chicken.")).not.toBeInTheDocument();
